...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (authError || !data.user) {
      ...
      if (IS_DEV) setDebug(d => ({ ...d, authError: authError?.message ?? 'no user returned' }));
      setLoading(false);
      return;
    }
    ...
    setSuccess(true);
    // Allow cookie to be written before navigating; use router.push so
    // the server middleware picks up the new session cookie.
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#050A14] flex items-center justify-center p-4 overflow-hidden relative">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="admin@trainyx.app" required
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl font-bold text-white text-sm tracking-wide transition-all disabled:opacity-50"
                style={{
                  background: loading ? 'rgba(27,111,235,0.5)' : 'linear-gradient(135deg, #1B6FEB, #1457C0)',
        ...
        {/* DEV-ONLY debug panel — stripped from production build */}
        {IS_DEV && Object.keys(debug).length > 0 && (
          <div className="fixed bottom-4 right-4 z-50 bg-black/90 border border-yellow-400/40 rounded-xl p-4 text-xs font-mono text-yellow-300 max-w-xs shadow-2xl">
            <p className="text-yellow-400 font-bold mb-2 text-sm">🛠 DEV AUTH</p>
            {Object.entries(debug).map(([k, v]) => (
              <p key={k}><span className="text-white/40">{k}: </span>{v}</p>
            ))}
          </div>
        )}
      </div>
